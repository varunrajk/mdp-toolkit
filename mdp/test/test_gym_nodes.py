...
from mdp.nodes import GymNode, GymContinuousExplorerNode
from ._tools import *
...
def test_gym_continuous_explorer_node():
    ...
    node = GymContinuousExplorerNode('MountainCarContinuous-v0', epsilon=0., decay=1.0)
    a = node.get_random_actions(1000)
    out = node.execute(a)
    assert_array_equal(out[:,node.observation_dim*2:node.observation_dim*2+node.action_dim], a)

    node = GymContinuousExplorerNode('MountainCarContinuous-v0', epsilon=1., decay=0.0)
    out = node.execute(a)
    assert_array_equal(out[:,node.observation_dim*2:node.observation_dim*2+node.action_dim], a)
